...
def find_nearest_neighbours_ReliefF_Discrete(x,y,neighbors,inst,data,distanceArray,maxInst):
    ...
    for j in range(maxInst):
        if inst != j:
            ...
            #using max index way more than needed,- looking ****ensure this is used by pete.
            if y[j]==y[inst]:
                max_index=find_max_index(D_hits) #two variable maxindex hits and max index misses - (if you do this then 
                #make list of hits and separate list of misses
                #take max of each set. Top 10)
