...
class ClassifierSet:
    ...
    def runGA(self, exploreIter, state, phenotype):
        ...
        # print "GA RUNNING ###############################################################"
        self.setIterStamps(
            exploreIter)  # Updates the iteration time stamp for all rules in the correct set (which the GA operates on).
        changed = False
        # -------------------------------------------------------
        ...
        if not cl1.equals(cl2) and random.random() < cons.chi:  # If parents are the same don't do crossover.
            ...
            changed = cl1.uniformCrossover(cl2, state,
                                           phenotype)  # PERFORM CROSSOVER!  #calls either the rule or tree crossover method depending on whether cl1 is tree or rule.
            ...
            while currentCount < 2:
                ...
                for j in posList:
                    if self.popSet[j].epochComplete:
                        ...
                        percentExperience = (exploreIter - self.popSet[j].initTimeStamp) / float(
    ...
    def runPopAveEval(self, exploreIter):
        ...
        # CHANGED_EVALUATION
        for cl in self.popSet:
            genSum += ((cons.env.formatData.numAttributes - len(cl.specifiedAttList)) / float(
    ...
    def getPopTrack(self, accuracy, exploreIter, trackingFrequency):
        ...
        trackString = str(exploreIter) + "\t" + str(len(self.popSet)) + "\t" + str(self.microPopSize) + "\t" + str(
            accuracy) + "\t" + str(self.aveGenerality) + "\t" + str(self.expRules) + "\t" + str(
        ...
        if cons.env.formatData.discretePhenotype:
            print(("Epoch: " + str(int(exploreIter / trackingFrequency)) + "\t Iteration: " + str(
                ...
                self.aveGenerality) + "\t ExpRules: " + str(self.expRules) + "\t Time: " + str(
                cons.timer.returnGlobalTimer())))
        # -------------------------------------------------------
        # CONTINUOUS PHENOTYPE
        # -------------------------------------------------------
