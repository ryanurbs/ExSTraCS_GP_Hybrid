...
class DataManagement:
    def __init__(self, trainFile, testFile, infoList = None):
        ...
        if cons.offlineData:
            ...
            #NEW############################################################################################################################################################
            self.classProportions = {}
            self.phenotypeRanked = None
            ...
            if cons.testFile == 'None': #If no testing data is available, formatting relies solely on training data.
                pass #data4Formating = rawTrainData
            else:
            ...
            #Fitness Update Discrete Endpoint ---------------------------------------------------------------------------
            self.ecFront = Pareto() #'ECFront'
            self.necFront = Pareto() #'NECFront'
            #self.bestCoverDiff = [0,0,1000,False,ENCFront,ECFront] #[bestcoverNotEpochComplete,bestcoverEpochComplete, bestruleAge, epochComplete, ENCFront, ECFront]

#             self.bestCoverDiff = {}
#             for each in self.phenotypeList:
#                 ECFront = Pareto(each, 'ECFront')
#                 ENCFront = Pareto(each, 'ENCFront')
#                 self.bestCoverDiff[each] = [0,0,1000,False,ENCFront,ECFront] #[bestcoverNotEpochComplete,bestcoverEpochComplete, bestruleAge, epochComplete]

#             self.maxFreq = []
#             self.minFreq = []
#             for i in range(0,self.specLimit+1):
#                 self.maxFreq.append(0)
#                 self.minFreq.append(1)
            ...
            #In preparation for later use by Turf - modify trainHeaderList to include only attribute labels.
            self.trainHeaderList.remove(cons.labelPhenotype)
            if self.areInstanceIDs:
                self.trainHeaderList.remove(cons.labelInstanceID)
        else:
            #Initialize global variables-------------------------------------------------
    ...
    def loadData(self, dataFile, doTrain):
        """ Load the data file. Check for and ignore any rows/instances that are missing an endpoint/phenotype/class value. Detect and store basic dataset parameters (i.e. number of attributes and number of instances)."""
        ...
        try:
            ...
            if doTrain:
                ...
                if cons.labelPhenotype in self.trainHeaderList:
                    ...
                    sys.exit() # HARD STOP - Abort algorithm run.
            ...
            for line in f:
                lineList = line.strip('\n').split('\t')
                #Check for and ignore any instances that are missing a phenotype value.  Not useful in supervised learning.
                if lineList[self.phenotypeRef] != cons.labelMissingData:
                    datasetList.append(lineList)
            f.close()
            if doTrain:
                self.numTrainInstances = len(datasetList)
...
#         print "DataManagement: Number of Instances = " + str(self.numTestInstances) #DEBUG
    ...
    def discriminateClasses(self, rawData):
        ...
        for each in list(self.classCount.keys()):
            ...
            #NEW############################################################################################################################################################
            self.classProportions[each] = (self.classCount[each] /float(total))
    ...
    def characterizeAttributes(self, rawData):
        ...
        for att in range(len(rawData[0])):
            if att != self.instanceIDRef and att != self.phenotypeRef:  #Get just the attribute columns (ignores phenotype and instanceID columns)
                stateDict = {}  #NEw AND NEW CODE WITHIN FOR FITNESS
                    ...
                    if inst in self.missingEndpointList: #don't use training instances without endpoint information.
                        ...
                        if target == cons.labelMissingData:
                            ...
                            if not self.attributeInfo[attributeID][0]: #If attribute is discrete
                                ...
                                if float(target) > self.attributeInfo[attributeID][1][1]:
                ...
                if not self.attributeInfo[attributeID][0]: #If attribute is discrete
                    instanceTotal = 0
                    for i in stateDict: #have to recount total instances in case of missing data.
                        instanceTotal += stateDict[i]
                    for i in stateDict: #have to recount total instances in case of missing data.
                        stateDict[i] = stateDict[i] / float(instanceTotal)

                    self.attributeInfo[attributeID][1] = stateDict
                    self.averageStateCount += len(stateDict)
        ...
        if self.continuousCount > 0:
            #self.binContinuous(rawData)
            pass

    #NEW############################################################################################################################################################
#     def binContinuous(self, rawData):
#         """ """
#         count = 0
#         for att in range(len(rawData[0])):
#             if att != self.instanceIDRef and att != self.phenotypeRef:  #Get just the attribute columns (ignores phenotype and instanceID columns)
#                 if self.attributeInfo[count][0]: #if continuous
#                     continuousBins = [0 for i in range(cons.numBins)]
#                     attMax = float(self.attributeInfo[count][1][1])
#                     attMin = float(self.attributeInfo[count][1][0])
#                     binInterval = (attMax - attMin) /float(cons.numBins)
#                     correction = 0 #Zero center the minimum
#                     if attMin < 0:
...
#                             pass
#                         else:
#                             ref = (float(target)+correction)/ float(binInterval)
#                             if int(ref) >= cons.numBins:
#                                 ref = cons.numBins - 1
#                             continuousBins[int(ref)]+= 1
#
#                     instanceTotal = 0
#                     for i in range(len(continuousBins)): #have to recount total instances in case of missing data.
#                         instanceTotal += continuousBins[i]
#                     for i in range(len(continuousBins)): #have to recount total instances in case of missing data.
#                         continuousBins[i] = continuousBins[i] / float(instanceTotal)
#
#                     self.attributeInfo[count].append(continuousBins)   #[continuousIdentifier,[min,max],[continuous bins]
#                 count += 1

    #############################################################################################################################################################
    def characterizePhenotype(self, rawData): #ContinuousCode #########################
        ...
        for inst in range(len(rawData)):
            target = rawData[inst][self.phenotypeRef]
            phenList.append(float(target))
        #print phenList
        self.phenotypeRanked = sorted(phenList)
        #print self.phenotypeRanked
        self.phenotypeList[0] = float(self.phenotypeRanked[0]) #minimum observed phenotype in training data
        self.phenotypeList[1] = float(self.phenotypeRanked[-1]) #maximum observed phenotype in training data

        self.phenSD = self.calcSD(self.phenotypeRanked)
        self.phenotypeRange = self.phenotypeList[1] - self.phenotypeList[0]
        #NEW############################################################################################################################################################
        self.phenotypeRandomPick = 0.5  #Experimental use -Follow up on this !!!!!
